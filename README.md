# Menstrual Hygiene Research Project

This repository contains the final research paper, statistical analysis, and supporting documents for a study of menstrual hygiene knowledge and practices among adolescent girls in sub-urban Lahore, Pakistan.

## Start Here

| Document | What it contains |
| --- | --- |
| **[Final research paper](RESEARCH_PAPER.docx)** | The completed paper in Microsoft Word format. |
| **[Research paper preview](RESEARCH_PAPER_RAW.md)** | The paper in a format that can be read directly on GitHub. |
| **[Plain scholarly paper](RESEARCH_PAPER%20-%20PLAIN%20SCHOLARLY.md)** ([DOCX](RESEARCH_PAPER%20-%20PLAIN%20SCHOLARLY.docx)) | Full paper in direct, restrained scholarly language. |
| **[Natural academic paper](RESEARCH_PAPER%20-%20NATURAL%20ACADEMIC.md)** ([DOCX](RESEARCH_PAPER%20-%20NATURAL%20ACADEMIC.docx)) | Full paper in natural academic prose with varied sentence rhythm. |
| **[Concise journal paper](RESEARCH_PAPER%20-%20CONCISE%20JOURNAL.md)** ([DOCX](RESEARCH_PAPER%20-%20CONCISE%20JOURNAL.docx)) | Full paper edited for a tighter journal style. |
| **[Simple results summary](docs/analysis/DATA_ANALYSIS_SUMMARY.md)** | The main findings and numbers in one short document. |
| **[Full analysis report](output/analysis_20260712_224950/analysis_report.md)** | The complete corrected results of the statistical analysis. |
| **[Analysis methodology](docs/analysis/ANALYSIS_METHODOLOGY.md)** | How the scores and statistical tests were chosen. |
| **[Comparative study](comparative_study/outputs/comparative_analysis.md)** | Verified comparison with 12 published studies. |
| **[Verification summary](docs/verification/VERIFICATION_SUMMARY.md)** | The checks performed to confirm the data and results. |

## Main Finding

The final analysis included **120 valid participants**.

- Practice scores differed across maternal-education groups at the unadjusted 0.05 level, but the result did not survive Holm correction for two principal outcomes.
- The omnibus knowledge-score comparison was not statistically significant; an exploratory ordinal trend was nominally significant.
- The effects were small, age may confound the group result, and some education groups had very few participants.

## Supporting Documents

- [Original study synopsis](doc.md)
- [Comparative evidence matrix](comparative_study/outputs/evidence_matrix.md)
- [Comparative research workspace](comparative_study/README.md)
- [Manuscript submission and revision plan](docs/verification/MANUSCRIPT_SUBMISSION_PLAN.md)
- [Reference verification](docs/verification/REFERENCES_VERIFICATION.md)
- [Analysis changes summary](docs/analysis/ANALYSIS_CHANGES_SUMMARY.md)
- [Complete list of generated output files](output/analysis_20260712_224950/FILE_INVENTORY.md)
- [Final charts and figures](figures/)
- [Final analysis output folder](output/analysis_20260712_224950/)

## Repository Layout

- `comparative_study/` contains downloaded papers, full-text evidence, metadata, comparison outputs, and audits.
- `docs/analysis/` contains statistical methods and result summaries.
- `docs/verification/` contains manuscript, reference, and integrity checks.
- `docs/technical/` contains developer and pipeline documentation.
- `figures/` contains manuscript-ready figures.
- `output/analysis_20260712_224950/` is the only retained, corrected final analysis bundle.

Participant-level scored CSV files are local analysis artifacts and are excluded from new commits. Public release requires a separate disclosure-control and ethics review.

## For Technical Users

Developers or researchers who need to run the analysis again can use the **[technical README](docs/technical/TECHNICAL_README.md)**. It contains installation instructions, data requirements, commands, output descriptions, testing, and troubleshooting.

## Research Team

- **Supervisor:** Dr Naureen Omar
- **Investigators:** Dr Ayesha Javed and Dr Fatima Sohail
