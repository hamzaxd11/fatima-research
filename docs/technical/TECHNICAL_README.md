...
For a detailed explanation of test selection (ANOVA vs. Kruskal-Wallis and assumption checks), please refer to [ANALYSIS_METHODOLOGY.md](../analysis/ANALYSIS_METHODOLOGY.md).
...
*   **Overall Scores**:
    *   Mean Knowledge Score: 6.65 (out of 9)
...
3. **Install required dependencies**
   ...
   - pyreadstat (≥1.2.0) - SPSS file reading
    ...
    - python-docx (≥1.1.0) - Manuscript DOCX generation
...
### Demographics
- `Age`: Respondent age
- `MotherEducation`: Maternal education level
- `FatherEducation`: Paternal education level
- `MotherOccupation`: Maternal occupation
- `FatherOccupation`: Paternal occupation
- `IncomePerMonth`: Monthly family income
- `NoOfFamilyMembersMale`: Number of male family members
- `NoOfFamilyMembersFeMale`: Number of female family members
...
- The nine exact Section III variable names listed under Data Requirements
- The seven exact Section IV variable names listed under Data Requirements
...
**If an unadjusted p < 0.05**:
- Report the observed association and its effect size without claiming causation.
- Check multiplicity-adjusted and sensitivity-analysis p-values before calling the result robust.
- Consider confounding, group imbalance, missing data, and instrument validity.

**If p ≥ 0.05**:
- State that the selected test did not detect a statistically significant difference.
- Do not interpret non-significance as proof of no relationship.
- Consider precision, sample size, data quality, and other plausible explanations.
