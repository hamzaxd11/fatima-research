...
This document records the scientific and editorial revision plan used to convert the current analysis package into a submission-ready manuscript without altering the underlying analytical results.
...
The final manuscript should reference only the consolidated final output bundle in `output/analysis_20260319_173428/`. Historical output folders were removed during consolidation so that only one final bundle remains in the repository.
...
Two items may still require author confirmation before journal submission.

The first is the reportable ethics approval number or formal approval wording, if the target journal requires it explicitly and it is not available in the current analysis bundle.

The second is target-journal formatting, including title-page affiliations, running title, corresponding author details, abstract word limits, and any journal-specific declaration language.
